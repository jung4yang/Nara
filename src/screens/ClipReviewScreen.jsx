...
export default function ClipReviewScreen({ selectedGenre, selectedFormat, onBack, onNext }) {
    ...
    const applyDislike = async () => {
        ...
        setLoadingCut(i);
        await delay(2000);
        setSrc((prev)   => prev.map((s, idx) => (idx === i ? altClips[i] : s)));
            ...
            <div className="sb-wrap">
                <div className="sb-header">
                    <div className="sb-title-area">
                        <div className="sb-title">클립 검토</div>
                        <div className="sb-desc">
                            {isCinematic
                                ? '시네마틱 트레일러용 9개 클립입니다. 마음에 들지 않으면 싫어요를 눌러 수정해주세요.'
                                : '숏폼 트레일러용 5개 클립입니다. 마음에 들지 않으면 싫어요를 눌러 수정해주세요.'}
                        </div>
                    </div>
                    <div className="sb-count">{liked.size} / {count} 확정</div>
                </div>
                ...
                <div className="sb-footer-hint">모든 클립에 좋아요를 누르면 최종 영상으로 넘어가요.</div>
            </div>
            ...
            <AnimatePresence>
                {panelCut >= 0 && (
                    ...
                    >
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 12, lineHeight: 1.5 }}>원하는 분위기, 속도, 색감 등을 적어주세요.</div>
                        <textarea
                            ...
                            placeholder="예: 카메라 무빙을 더 느리게, 색감을 더 어둡게"
                            value={panelValue}
